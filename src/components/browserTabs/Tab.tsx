    ...
    return (
        <div className={`${classes.tabBox} ${activeTabIndex === index && classes.activeTabBox}`}>
            <div className={classes.tab}>
                {/* FIXME: NEED TO WORK */}
                <div role="presentation" onClick={() => updateActiveTabIndex(index)} className={classes.nameWrapper}>
                    <span className={classes.name}>{name.length > 9 ? `${name.substring(0, 9)}...` : name}</span>
                </div>
                {/* FIXME: NEED TO WORK */}
                <div className={classes.crossWrapper} role="presentation" onClick={() => closeTab(index)}>
                    <Cross />
                </div>
            </div>
        </div>
